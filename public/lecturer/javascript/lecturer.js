$(function () {
		...
		question_quizz(data) {
			...
			const quizz = {
				...
				sendDataToStudent() {
					...
					$.post(ROOT + "quizzLecturer/insertScoreLecturer", result,
						function (data, textStatus, jqXHR) {

						},
						"json"
					);
					$.post(ROOT + "quizzLecturer/insertLecturerSubject", { id_subject: _this.infor.id_subject },
						function (data, textStatus, jqXHR) {
					...
					$('.expand .courses .start-education .turn-back').on('click', function () {
						window.location.href = ROOT + "quiz/lecturer";
					});
				},
	...
	const callAPIResult = {
		call() {
			const _this = this;
			$.post(ROOT + "quizzLecturer/callAPIResult", {},
				function (data, textStatus, jqXHR) {
					_this.render(data);
				},
				"json"
	...
	const profile = {
		...
		changePassword() {
			...
			validate({
				...
				callback(forms) {
					$.post(ROOT + "quizzLecturer/changePasswordLecturer", forms,
						function (data, textStatus, jqXHR) {
							if (data) {
		...
		changeProfile() {
			...
			validate({
				...
				callback(forms) {
					...
					$.post(ROOT + "quizzLecturer/updateLecturer", forms,
						function (data, textStatus, jqXHR) {
							console.log(data)
			...
			let html = data.map(item => `
				<div data-id="${item.id_subject}" class="item">
						...
						<div class="content-subject">
							...
							<h1 class="title">${item.subject_name}</h1>
							<p class="description">${item.description}</p>
						</div>
						<div class="info">
							<p class="eudcation">Trình độ: <span>${item.education_name}</span></p>
							<p class="user_name">Họ và tên giảng viên phụ trách: <span>${item.user_name}</span></p>
